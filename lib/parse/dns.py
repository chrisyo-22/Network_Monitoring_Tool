...
def _parse_rr(raw_data, start):
        ...
        if rr_type == 1:
            ...
            rr_cname, pos = _parse_name(raw_data, pos)
        else:
            pos += rr_rdlength 

        return {'name': name, 'type': rr_type, 'class': rr_class, 'ttl': rr_ttl, 'address': rr_addr, 'cname': rr_cname}, pos
...
class DNS:
    ...
    def __str__(self):
        ...
        for answer in self.answers:
            info += '\t\t\t - Name: {}, Type: {}, Class: {}, TTL: {}, Address: {}, CName: {}\n'.format(
                answer['name'], answer['type'], answer['class'], answer['ttl'], answer['address'] or "N/A", answer['cname'] or "N/A"
            )
        return info
